...
<script>
  ...
  export default {
    name: "upload-Gift-Card-Sales-Component",
    components: {},
    data() {
      return {
    ...
    methods: {
      ...
      uploadGiftReportCSV() {
          ...
          if (this.giftReportFormData != null) {
              ...
              this.overlay = true;
              databaseUploadService.uploadGiftCardSales(this.giftReportFormData).then((response) => {
                  if (response.status == 200) {
                      this.overlay = false;
                      alert("Successfully uploaded gift card sales report to database")
